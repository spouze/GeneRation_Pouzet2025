...
if (output.file==""){output.file="simulationburn"}
...
#  invisible(WNetwork(relinit, tkplot = FALSE)) ; invisible(dev.off())
...
system(paste("echo 1ST ADAPTATION COMPLETED", sep=""))
system(paste("echo  ", sep=""))
...
#system(paste("cd ", date, output.file, sep=""))
system(paste("cp ",date, output.file, "/", date, output.file, ".param ", date, output.file, ".burnin.param ", sep=""))
paramfile=paste(date, output.file, ".burnin.param", sep="")
theta = gv("FITNESS_OPTIMUM", read.param(paramfile))
strength=gv("FITNESS_STRENGTH", read.param(paramfile))
n=length(which(strength != 0))
newtheta=theta
for (i in which(strength != 0)){
	newoptimum=theta[i]
	s=strength[i]
	d=sqrt(1/(n*s)*log(2))
	newoptimum = newoptimum + d*sample(c(1, -1), 1) #soit on ajoute, soit on retire
	if (newoptimum < 0){newoptimum = newoptimum + 2*d}
	if (newoptimum > 1){newoptimum = newoptimum - 2*d}
	newtheta[i] = newoptimum
}
###############################################
#newtheta=theta=runif(length(newtheta)) ## Pour le moment, ca pb w calculation: d = 1.86
###############################################
paramms  <- readLines(paramfile)
newtheta=paste(newtheta, collapse = ' ')
paramms[which(startsWith(paramms, "FITNESS_OPTIMUM"))]=paste("FITNESS_OPTIMUM\t", newtheta)
writeLines(paramms, con=paramfile)
system("echo Fitness optima changed for the burnin.")

#lancer la nouvelle simulation.

system(paste("mkdir ", date, output.file, "/", date, output.file, ".burnin", sep=""))
#system(paste("./Launch_GeneRation.R ",
#								"-p ",   date, output.file, "/", date, output.file, ".burnin.param ",
#								"-ipop ",date, output.file, "/", date, output.file, ".finalpop.rds",
#								sep=""))

param.file2=paste(date, output.file, ".burnin.param", sep="")
init.pop2=  paste(date, output.file, "/", date, output.file, ".finalpop.rds", sep="")
output.file2=paste(output.file, ".burnin", sep="")
cat("Running burnin simulation ",date, output.file2, ": \n",sep="")
sim2 <- launchprogevol(paramfile = param.file2, initialpop=list.load(init.pop2))
#OUTPUTS
write.table(sim2[[1]], file=paste(date, output.file2, ".table", sep=""), sep="\t", quote=FALSE, row.names=FALSE)
list.save(sim2[[2]], file=paste(date, output.file2, ".initialpop.rds", sep=""))
list.save(sim2[[3]], file=paste(date, output.file2, ".finalpop.rds", sep=""))
#move files to folder
system(paste("mv ",
							date, output.file2, ".table ",
							date, output.file2, ".initialpop.rds ",
							date, output.file2, ".finalpop.rds ",
							date, output.file2, ".param ",
							date, output.file, "/", date, output.file2,
							sep=""))

# simu=LoadSimu(paste(date, output.file, "/",date, output.file2, "/", sep=""))
# png(paste (date, output.file, "/",date, output.file2, "/", simu[[4]], ".00.graphall", sep=""), width=1000, height=600)
# 	graphall(simu[[1]]) ; invisible(dev.off())
# 	relinit=Quantifycoding(simu, param = simu[[5]] ,reference ="INIT", write=F)$relevant
# png(paste (date, output.file, "/",date, output.file2, "/", simu[[4]], ".04.relevant_toInit.heatmap", sep=""), width=700, height=700)
#   WTab(relinit, cellnote = TRUE, main="Relevant matrix compared to Init", devnew=F) ; invisible(dev.off())
#relinit=Quantifycoding(simu, param = param ,reference ="INIT", write=F)$relevant
#png(paste (date, output.file, "/", simu[[4]], ".04.relevant_toInit.network", sep=""), width=700, height=500)
#  invisible(WNetwork(relinit, tkplot = FALSE)) ; invisible(dev.off())
...
simu=LoadSimu(paste(date, output.file, "/",date, output.file2, "/", sep=""))
png(paste (date, output.file, "/",date, output.file2, "/", simu[[4]], ".00.graphall.png", sep=""), width=1000, height=600)
  graphall(simu[[1]])
  invisible(dev.off())
  relinit=Quantifycoding(simu, param = simu[[5]] ,reference ="INIT", write=F)$relevant
  invisible(dev.off())
  
png(paste (date, output.file, "/",date, output.file2, "/", simu[[4]], ".01.WInit.png", sep=""), width=1000, height=450)
  par(mfrow=c(1,2))
  W = WFinale(output=simu[[1]], gen=1)
  WTab3(W, cellnote = T, from = T, round_nb = 2, on = T)
  options(warn=-1) ; WNetwork4(W) ; invisible(dev.off()) ; options(warn=0)

png(paste (date, output.file, "/",date, output.file2, "/", simu[[4]], ".02.WFinal.png", sep=""), width=1000, height=450)
  par(mfrow=c(1,2))
  W = WFinale(output=simu[[1]])
  WTab3(W, cellnote = T, from = T, round_nb = 2, on = T)
  options(warn=-1) ; WNetwork4(W) ; invisible(dev.off()) ; options(warn=0)

png(paste (date, output.file, "/",date, output.file2, "/", simu[[4]], ".03.WFinal_Kinetics.png", sep=""), width=750, height=450)
  W = WFinale(output=simu[[1]])
  WKinetics3(W, end = 19, optima = gv("FITNESS_OPTIMUM", simu[[5]])[which(gv("FITNESS_STRENGTH", simu[[5]])!=0)])


#Message de courtoisie pour la sortie
system(paste("echo Final fitness of pop: ", round(sim2[[1]]$MFit[nrow(sim[[1]])],3), sep=""))
system(paste("echo 2ND ADAPTATION COMPLETED", sep=""))
system(paste("echo  ", sep=""))
end=format(Sys.time(), "%m%d_%H%M%S")
system(paste("echo JOB COMPLETED at ", end, ".", sep=""))
system(paste("echo  ", sep=""))
